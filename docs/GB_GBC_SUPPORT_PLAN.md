...
| 8×8 pixel encoding | NES CHR 2bpp | **GB 2bpp**: still 16 bytes per tile, but each **row** is a byte pair (bit planes **interleaved per line**). Same four color indices 0–3 per pixel; wrong layout = “snow” or diagonal garbage if you interpreted it as NES. |
...
| Sprites | NES OAM | **40 sprites × 4 bytes** in OAM; tiles still 8×8 or 8×16—Step 2 when wiring drops to sprite windows. |
| Resolution / grids | 256×240 (tooling often 256×256) | Playfield **160×144**; not required for CHR bank painting, but useful context for later layout windows. |
