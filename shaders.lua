grayScaleShader = love.graphics.newShader[[
	vec4 effect(vec4 loveColor, Image tex, vec2 textureCoords, vec2 screenCoords) {
    vec4 pixel = Texel(tex, textureCoords); // get the current pixel color
    float gray = dot(pixel.rgb, vec3(0.299, 0.587, 0.114)); // convert to grayscale
    return vec4(gray, gray, gray, pixel.a); // return the grayscale color with original alpha
	}
]]
...
darkerImageShader = love.graphics.newShader[[
	vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
		...
		textureColor.rgb *= 0.6; // Change 0.5 to another value if you want it darker or lighter
...
compositeScanlinesShader = love.graphics.newShader[[
    ...
    vec4 effect(vec4 sys_color, Image texture, vec2 texture_coords, vec2 screen_coords) {
      ...
      for (int n = -2; n < 2; n++) {
        ...
        float cx = texture_coords.x * screen.x + float(n);
        float phase = cx * TAU / 4.0;
      ...
      float row = floor(texture_coords.y * screen.y + 1e-5);
      float scanline = clamp(mod(row, 2.0) * 0.2 + 0.8, 0.0, 1.0);
      pixel.rgb *= scanline;
