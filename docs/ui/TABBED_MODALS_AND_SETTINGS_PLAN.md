...
- **Logical layout** (from mock): column headers **Dark mode** | **Light mode**; row labels **Focused**, **Non-focused**, **Text/Icons**; one **swatch** per theme x row (six slots). Until the picker exists, swatches are **read-only placeholders** (click may no-op or reserved for future `onRequestColorEdit`).
...
3. Appearance tab: implement the **mock layout inside the shared 3 x R panel** (swatches + padding rows as needed) + stub persistence / `app_colors` merge.  
