...
describe("bank_view_controller.lua", function()
  ...
  it("builds one bank-window layer per bank and lazily resolves tiles for the current bank", function()
    ...
    expect(win.title).toBe("Bank 2/3")
  ...
  it("chrOrderingIndexForGridPos matches CHR bank oddEven mapping for 8x16 / oddEven layout", function()
    expect(BankViewController.chrOrderingIndexForGridPos("normal", 1)).toBe(1)
    expect(BankViewController.chrOrderingIndexForGridPos("8x8", 33)).toBe(33)
    expect(BankViewController.chrOrderingIndexForGridPos("oddEven", 33))
      .toBe(BankViewController.chrOrderingIndexForGridPos("8x16", 33))
