...
describe("toolbar_controller.lua - palette link handle", function()
  ...
  it("creates a specialized toolbar without a link handle for a single generic palette window", function()
    ...
    expect(win.specializedToolbar.compactButton.visible).toBe(false)
  end)
  ...
  it("creates a specialized toolbar with a link handle for ROM palette windows", function()
    ...
    expect(win.specializedToolbar.compactButton).toBeTruthy()
    expect(win.specializedToolbar.compactButton.visible).toBe(false)
    expect(win.specializedToolbar.buttons[3]).toBe(win.specializedToolbar.compactButton)
    expect(win.specializedToolbar.resetCellButton).toBeTruthy()
    expect(win.specializedToolbar.buttons[4]).toBe(win.specializedToolbar.resetCellButton)
    expect(win.specializedToolbar.resetAllButton).toBeTruthy()
    expect(win.specializedToolbar.buttons[5]).toBe(win.specializedToolbar.resetAllButton)
    expect(win.specializedToolbar.linkButton).toBeTruthy()
    expect(win.specializedToolbar.buttons[6]).toBe(win.specializedToolbar.linkButton)
  end)
